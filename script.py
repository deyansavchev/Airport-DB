...
# --- ФУНКЦИИ ЗА ТЪРСЕНЕ ---
def search_flights():
    # 1. Изчистваме старите резултати от таблицата
    for row in tree.get_children():
        tree.delete(row)

    # 2. Взимаме данните от интерфейса
    ...
    # 1 = Двупосочен (True), 2 = Еднопосочен (False)
    ...
    if conn:
        cursor = conn.cursor()
        try:
            # 3. Дефинираме SQL заявката с новата колона is_round_trip
            query = """
            ...
            # 4. Обработка на резултатите за визуализация
            for i, row in enumerate(results):
                # Превръщаме кортежа в списък, за да променим True/False на текст
                display_row = list(row)

                # display_row[7] е колоната is_round_trip
...
def on_tree_select(event):
    ...
    entry_flight.delete(0, tk.END)
    entry_flight.insert(0, flight_data[0])  # ID е в първата колона

    entry_price.delete(0, tk.END)
    entry_price.insert(0, flight_data[6])  # Цената е в седмата колона (индекс 6)
